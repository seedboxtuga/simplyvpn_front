...
/** Build a safe backend URL whether env has scheme or not */
  ...
  try {
    ...
    if (!API_KEY) {
      return NextResponse.json(
        { ok: false, status: 500, error: "Server misconfigured: SIMPLYVPN_API_KEY missing" },
        { status: 500 }
      )
    }
    if (!country) {
      return NextResponse.json(
        { ok: false, status: 400, error: "Missing 'country' in body" },
        { status: 400 }
      )
    }
    ...
    console.log("[SimplyVPN] Requesting config:", {
    ...
    if (backendData?.ok && backendData?.config) {
      return NextResponse.json(
        { ok: true, status: 200, config: backendData.config },
        { status: 200 }
      )
    }

    return NextResponse.json(
      { ok: false, status: 500, error: "Invalid backend response", details: backendData },
      { status: 500 }
    )
  } catch (error: any) {
    console.error("[SimplyVPN] API route error:", error)
    return NextResponse.json(
      { ok: false, status: 500, error: "Failed to generate config", details: error?.message ?? String(error) },
      { status: 500 }
    )
  }
}
