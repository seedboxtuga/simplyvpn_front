...
## File: components/MiniKitProvider.tsx
\`\`\``typescript
"use client";
...
Minimal Next.js (App Router) app using `@worldcoin/minikit-js@1.9.6`.

UI has **only one button**. After successful **Orb-level verification**,  
a blurred spoiler with text `"world id verified"` appears — tap to reveal.

## Quickstart
\`\`\`bash
npm install
cp .env.local.example .env.local   # set APP_ID and NEXT_PUBLIC_ACTION_ID
npm run dev
\`\`\``
